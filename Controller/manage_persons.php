...
<body>
	...
	<div class="containers">
		<div class="sidebar">
			...
			<ul class="navbar">
				...
				<li><a class="selected" href="manage_persons.php"><i class="fa fa-users" aria-hidden="true"></i>&nbsp;&nbsp;Manage Persons</a></li>
		...
		<div class="content">
			...
			<div class="box">
				<div class="box-top">
					<h4 class="text-center">MANAGE PERSONS</h4>
				</div>
				<div class="filter">
					<input type="text" id="search_person" name="search" placeholder="Search..">
				</div>	
			</div>


			<table class="table table-bordered statistic_tbl" id="statistic_tbl_person">
			  <tr class="active">
				...
				<?php
					$query_person="SELECT `id`, `first_name`, `last_name`, `email`, `country`, `info_completion` FROM `users` LIMIT 8";
					$perform_query_person=mysqli_query($connect,$query_person);
					while($result_p=mysqli_fetch_assoc($perform_query_person))
					{
						echo "<tr>";
						// <!-- name -->
						echo "<td>".$result_p['first_name'].' '.$result_p['last_name']."</td>";
						// <!-- Email -->
						echo "<td>".$result_p['email']."</td>";
						// <!-- Country -->
						echo "<td>".$result_p['country']."</td>";
						// <!-- info_completion -->
						echo "<td>".$result_p['info_completion']."&nbsp;%</td>";
						// <!-- More Details -->
						echo "
						<td><a href='#' class='details'>More Details </a>
						<input type='hidden' class='person_id' value='".$result_p['id']."'/>
						</td>
						";
						echo "</tr>";
					}
	...
	<!-- pop up of logout model -->
    <!-- Modal -->
	<div class="modal fade fading_opacity_panel" id="display_individual_person" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	  <button type="button"  class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	  <div class="modal-dialog mod_dio_panel" role="document">
	...
	<script type="text/javascript">
    	...
    	$('.details').click(function(){
    			...
    			$('#display_individual_person').modal('show');	
    	});
    	...
    	function get_person_details(person_id)
    	{
	        ...
	        var url = "get_user_more_details.php";
	        var vars = "person_id="+person_id;
		    ...
		    if(hr.readyState == 4 && hr.status == 200) {
			    ...
			    if(jArraygetpersondetails!=null)
				{
					...
					$.each( jArraygetpersondetails, function( key, value ){
						...
						//person
						$(".fading_opacity_panel .mod_dio_panel .mod_con_panel .mod_body_persons_panel .person_detials").append("<img src=\"../"+value.photo+"\">&nbsp;&nbsp;<h4>"+value.name+"</h4><div class=\"element\"><span class=\"title\">Email:</span><span>"+value.email+"</span></div><div class=\"element\"><span class=\"title\">Address:</span><span>"+value.adderss+"</span></div><div class=\"element\"><span class=\"title\">Country:</span><span>"+value.country+"</span></div><div class=\"element\"><span class=\"title\">City:</span><span>"+value.city+"</span></div><div class=\"element\"><span class=\"title\">Description:</span><p>"+value.description+"</p></div><div class=\"element\"><span class=\"title\">Trusted:</span><span>"+trusted+"</span></div><div class=\"element\"><span class=\"title\">Telephone 1:</span><span>"+tele1+"</span></div><div class=\"element\"><span class=\"title\">Telephone 2:</span><span>"+tele2+"</span></div>");
		...
		function get_person_search(search)
    	{
	        ...
	        // Create some variables we need to send to our PHP file    
	        var search=search;
	        var url = "get_user_more_details_search.php";
		    ...
		    if(hr.readyState == 4 && hr.status == 200) {
			    ...
			    if(jArraygetpersondetails_search!=null)
				{
					...
					$.each( jArraygetpersondetails_search, function( key, value ){
						...
						//person
						$("#statistic_tbl_person").append("<tr><td>"+value.name+"</td><td>"+value.email+"</td><td>"+value.country+"</td><td>"+value.info_completion+"&nbsp;%</td><td><a href='#' class='details'>More Details </a><input type='hidden' class='person_id' value='"+value.id+"'/></td></tr>");
					...
					$('.details').click(function(){
						...
						$("#statistic_tbl_person").html("<td colspan='5'><h4 class='text-center' style='color:#ff0000'>There's No Result for Your Query...</h4></td>");
				}
	            ...
	            $("#statistic_tbl_person").html("<tr class=\"active\"><td><i class=\"fa fa-edit\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Name</td><td><i class=\"fa fa-envelope-o\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Email</td><td><i class=\"fa fa-university\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Country</td><td><i class=\"fa fa-info\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Info_Completion</td><td><i class=\"fa fa-info\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Others</td></tr>");
	            $("#statistic_tbl_person").append("<tr><td colspan='5'><h4 class='text-center'>There's No Result for Your Query...</h4></td></tr>");
	        }
	        }
	        // Send the data to PHP now... and wait for response to update the status div
	        hr.send(vars); // Actually execute the request
