...
<body>
	...
	<div class="containers">
		<div class="sidebar">
			...
			<ul class="navbar">
				...
				<li><a class="selected" href="announcement.php"><i class="fa fa-clone" aria-hidden="true"></i>&nbsp;&nbsp;Manage Announcement</a></li>
		...
		<div class="content">
			...
			<div class="box">
				<div class="box-top">
					<h4 class="text-center">MANAGE ANNOUNCEMENT &nbsp;&nbsp;(<i class="fa fa-bullhorn" aria-hidden="true"></i>)</h4>
				</div>
				<div class="filter">
					<input type="text" name="search" id="search_announcement" placeholder="Search By Post Owner ...">
				</div>	
			</div>

			<table class="table table-bordered statistic_tbl" id="statistic_tbl_announcement_owner">
			  
			  <tr class="active">
			  	<td><i class="fa fa-edit" aria-hidden="true"></i>&nbsp;&nbsp;Post Owner</td>
			  	<td><i class="fa fa-flag" aria-hidden="true"></i> Country</td>
			  	<td><i class="fa fa-eye" aria-hidden="true"></i>&nbsp;&nbsp;Last Seen</td>
			  ...
			  <?php
			  	$query_announcement="SELECT `id`, `account_id`, `account_type`, `country`, `lastSeen`, `date` FROM `request_post` LIMIT 8";
			  	$perform_query_anouncement=mysqli_query($connect,$query_announcement);
			  	while($result_announcement=mysqli_fetch_assoc($perform_query_anouncement))
			  	{
			  		$account_id=$result_announcement['account_id'];
		  			$account_type=$result_announcement['account_type'];
		  			if($account_type=='p')
		  			{
				  	...
				  	echo "<td>".$result_announcement['country']."</td>";
				  	echo "<td><b>".$result_announcement['lastSeen']."</b></td>";
				  	echo "<td>".$result_announcement['date']."</td>";
				  	echo "<td>
				  			<button class=\"btn btn-danger delete_announcement\" >DELETE ANOUNCEMENT</button>
				  			<input type=\"hidden\" class=\"announcement_id\" value=\"".$result_announcement['id']."\"/>
				  			<input type=\"hidden\" class=\"account_id\" value=\"".$result_announcement['account_id']."\"/>
				  			<input type=\"hidden\" class=\"account_type\" value=\"".$result_announcement['account_type']."\"/>
				  		 </td>";
				  	echo "<td><a href=\"../all_request_posts/display_request_post.php?requets_num=".$result_announcement['id']."\">More Details</a></td>";
			  		echo "</tr>";
			  	}
			  ?>
	...
	<script type="text/javascript">
		...
		$('.delete_announcement').click(function(){
    		...
    		var announcement_id = $(this).siblings('.announcement_id').val();
    		var account_id = $(this).siblings('.account_id').val();
    		var account_type = $(this).siblings('.account_type').val();
    		
    		delete_announcement(announcement_id,account_id,account_type);
    	});

    	function delete_announcement(announcement_id,account_id,account_type)
    	{
	        ...
	        var url = "delete_announcement.php";
	        var vars = "announcement_id="+announcement_id+"&account_id="+account_id+"&account_type="+account_type;
    	...
    	$("#search_announcement").on("keyup",function(){
    		var search=$(this).val();
    		// alert(search);	
    		get_announcement_owner_search(search);
    	});

    	function get_announcement_owner_search(search)
    	{
    		// Create our XMLHttpRequest object
	        var hr = new XMLHttpRequest();
	        // Create some variables we need to send to our PHP file    
	        var search=search;
	        var url = "get_announcement_owner_search.php";
		    ...
		    if(hr.readyState == 4 && hr.status == 200) {
			    ...
			    if(jArraygetpersonpostowner_search!=null)
				{
					...
					//console.log(jArraygetpersonpostowner_search);	
					$("#statistic_tbl_announcement_owner").html('');
					$("#statistic_tbl_announcement_owner").html("<tr class=\"active\"><td><i class=\"fa fa-edit\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Post Owner</td><td><i class=\"fa fa-flag\" aria-hidden=\"true\"></i> Country</td><td><i class=\"fa fa-eye\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Last Seen</td><td><i class=\"fa fa-clock-o\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Date</td><td><i class=\"fa fa-street-view\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Action</td><td><i class=\"fa fa-info\" aria-hidden=\"true\"></i>&nbsp;&nbsp;Others</td></tr>");
					...
					$.each( jArraygetpersonpostowner_search, function( key, value ){
						...
						$("#statistic_tbl_announcement_owner").append("<tr><td>"+value.account_name+"</td><td>"+value.country+"</td><td><b>"+value.announcement_latSeen+"</b></td><td>"+value.announcement_date+"</td><td><button class=\"btn btn-danger delete_announcement\" >DELETE ANOUNCEMENT</button><input type=\"hidden\" class=\"announcement_id\" value=\""+value.announcement_id+"\"/><input type=\"hidden\" class=\"account_id\" value=\""+value.announcement_account_id+"\"/><input type=\"hidden\" class=\"account_type\" value=\""+value.announcement_account_type+"\"/></td><td><a href=\"../all_request_posts/display_request_post.php?requets_num="+value.announcement_id+"\">More Details</a></td></tr>");					
					...
					$("#statistic_tbl_announcement_owner").html("<td colspan='5'><h4 class='text-center' style='color:#ff0000'>There's No Result for Your Query...</h4></td>");
				}
	            ...
	            $("#statistic_tbl_announcement_owner").append("<tr><td colspan='5'><h4 class='text-center'>There's No Result for Your Query...</h4></td></tr>");
	         }
