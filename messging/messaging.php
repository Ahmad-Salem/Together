...
<body>
			...
			<div class="col-md-4 col-sm-4 col-xs-8">
				<div class="options">
					<ul class="list-unstyled option">
						...
						<li><a href="messaging.php"><i class="fa fa-envelope-o fa-2x" aria-hidden="true"></i></a></li>
	...
	<div class="container message">
		...
		<div class="chatbox">
			...
			<?php if(!isset($_GET['uid']) && empty($_GET['uid']) && !isset($_GET['gender']) && empty($_GET['gender']) && !isset($_GET['acc_type']) && empty($_GET['acc_type'])):?>
			...
			<?php 
				...
				{
					$uid=$_GET['uid'];
					$query_msg_p="SELECT `first_name`,`last_name`,`photo` FROM `users` WHERE id='$uid' LIMIT 1";
					$perform_query_msg_p=mysqli_query($connect,$query_msg_p);
					$result_msg_p=mysqli_fetch_assoc($perform_query_msg_p);
				}
			?>
			<?php if($perform_query_msg_p):?>
			<div class="chat_process_container">
				...
				<div class="header">
					...
					?>
					<div class="user-photo"><img src="<?php echo $image;?>"  title="<?php echo $result_msg_p['first_name'].' '.$result_msg_p['last_name']; ?>" /></div>
					<p class="chat-message"><?php echo $result_msg_p['first_name'].' '.$result_msg_p['last_name'];?></p>
				</div>
				
				<input type="hidden" id="user_id" name="" value="<?php echo $uid; ?>" />
			...
			<div class="messge_chat">	
					...
					<button class="messge_chat_btn" id="message_box_p_p" ><span><i class="fa fa-reply fa-lg" aria-hidden="true"></i></span></button>
	...
	<script type="text/javascript" src="../javascript/messaging.js"></script>
