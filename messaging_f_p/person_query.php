<?php
	...
	$query_person="SELECT `id`,CONCAT(`first_name`,'    ',`last_name`) `fullname` ,`photo`,`gender` FROM users WHERE `first_name`  LIKE '".$search_input."%' OR `last_name`  LIKE '".$search_input."%' ";
	...
	if(mysqli_num_rows($perform_query_person)!=0 && $search_input!=null)
	{
		...
		while($result_person=mysqli_fetch_assoc($perform_query_person))
		{
			...
			.$result_person['fullname'].
			"</a> </li>";
			echo "<button id=\"tooltip_msg_id\" style=\"margin-bottom:10px; margin-top:10px;\" type=\"button\" class=\"btn btn-default btn-block tooltip_msg\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Tooltip on bottom\">Say Hello To ".$result_person['fullname']."</button>";
		}
