<?php
	...
	if (isset($_POST['get_like_post_id'])) {
		...
		$query_get_accounts="SELECT `id`, `account_id`, `comment`, `post_id`, `date`, `account_type` FROM `comments_normal_post` WHERE `post_id`='$post_id' ";
		...
		{
			$comment=$result_get_accounts['comment'];
			...
			}else if($result_get_accounts['account_type']=='f')
			{
				//foundation
