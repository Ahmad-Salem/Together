	...
	if($_COOKIE['Account_type']=='person')
		{
			...
			}else if($_COOKIE['Account_type']=='foundation')
			{
				...
				$foundationo_id=$_COOKIE['foundation_id'];
				$Acc_type='f';
				$query_normal_posts="SELECT `id`, `account_id`, `caption`, `date` FROM `normal_posts`  WHERE `account_id`='$foundationo_id' AND `account_type`='$Acc_type' ORDER BY id DESC LIMIT 5";
				$perform_normal_query=mysqli_query($connect,$query_normal_posts);
				...
				while($row=mysqli_fetch_assoc($perform_normal_query))
				{
					...
					"
						...
						<div class=\"post\">
							...
							<div class=\"post_header\">
								<img class=\"img-responsive\" src=\"$image\" />
								<a href=\"posting_person/display_normal_post.php?post_no={$post_id}&&account_id={$foundation_id}&&account_type=f\"> <h5> $foundation_name &nbsp;(MY Profile)  </h5></a>
								<span><b>$date_ago</b></span>
								<div class=\"clear\"></div>
							</div>
							<!-- image of the post -->
								...
								while($result_N_P_f=mysqli_fetch_assoc($perform_normal_posts_images))
								{
									...
									}else if($count==1)
									{
										...
										<div class=\"rest_photos\">
											<div class=\"R_photo\">
												<img class=\"img-responsive\" src=\"postingattachment/foundation/$foundation_id/$post_id/{$result_N_P_f['images']}\" />
												<a href=\"posting_person/display_normal_post.php?post_no={$post_id}&&account_id={$foundation_id}&&account_type=f\" > <span class=\"number_rest_images\">+2<span> </a>
											</div>
							...
							";
								...
								$query_foundation_comments_number="SELECT `id` FROM `comments_normal_post` WHERE `post_id` = '$post_id'  ";
								@$perform_query_foundation_comments_number=mysqli_query($connect,$query_foundation_comments_number);
								@$comments_f_number=mysqli_num_rows($perform_query_foundation_comments_number);
								if($perform_query_foundation_likes_number&&$perform_query_foundation_comments_number)
									{
										echo "	
											<div class=\"likes\">
												<span><b id=\"likes_f{$post_id}\" >{$liked_f_number}</b></span><span>&nbsp;&nbsp;<a href=\"#\" class=\"liked_pop_up\" >likes</a></span>
												&nbsp;&nbsp;&nbsp;<span><b id=\"comments_f{$post_id}\">{$comments_f_number}</b></span><span>&nbsp;&nbsp;<a href=\"#\" class=\"comments_pop_up\">Comments</a></span>
												<input type=\"hidden\" class=\"post_id_get_likes\" value=\"$post_id\" />
											</div>
											";
