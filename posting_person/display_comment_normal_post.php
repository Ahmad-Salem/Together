<?php
	...
	$query_comment_normal_post="SELECT comments_normal_post.account_id, `comment`, `post_id`, comments_normal_post.date as `date` , comments_normal_post.account_type FROM `comments_normal_post` INNER JOIN `normal_posts` ON comments_normal_post.post_id = normal_posts.id ";
	...
	while($result_comment=mysqli_fetch_assoc($perfrom_query_comment_normal_post))
	{
		 if($result_comment['account_type']=='p')
		 {
			...
			// $Account_info['gender']=$account_gender;
			// $Account_info['name']=$Account_name;
			// $Account_info['account_type']="p";
			// $Account_info['account_id']=$person_id;
			// $Account_info['post_id']=$result_comment['post_id'];
			// $Account_info['comment']=$result_comment['comment'];
			// $Account_info['date']=time_elapsed_string($result_comment['date'], $full = false);
			
		 	
		 }else
		 {
		 	//foundation
			$foundation_id=$result_comment['account_id'];
			...
			// $Account_info['image']=$image;
			// $Account_info['name']=$Account_name;
			// $Account_info['account_type']="f";
			// $Account_info['account_id']=$foundation_id;
			// $Account_info['post_id']=$result_comment['post_id'];
			// $Account_info['comment']=$result_comment['comment'];
			// $Account_info['date']=time_elapsed_string($result_comment['date'], $full = false);
